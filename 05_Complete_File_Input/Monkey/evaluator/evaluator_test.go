...
// test evaluation of object.Integer returning expressions
...
// test evaluation of object.Boolean returning expressions
...
// see extended TestEvalIntegerExpression(t *testing.T) as it returns object.Integer
...
// test evaluation for error handling
...
// test evaluation of object.Function
...
// test evaluation of closures
// To answer the question : "Why extend the function’s environment and not the current environment?"
...
// test evaluation of object.String returning expressions
...
// test evaluation of string concatenation
...
func TestStringComparison(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not Booolean. got=%T (%+v)", evaluated, evaluated)
	}
	if obj.Value != true {
		t.Errorf("object has wrong value. got=%t", obj.Value)
	}
}

// test evaluation of Buildin Functions
...
func TestArrayLiterals(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("array has wrong number of arguments. got = %d", len(result.Elements))
	}
...
// test evaluation of array index operator expressions
	...
	for _, tt := range tests {
		...
		if ok {
			...
			if tt.expected != evaluated.Inspect() {
				t.Errorf("Wrong error message, expected = %s. got = %s", tt.expected, evaluated.Inspect())
			}
		}
	}
}

// test evaluation of hash literals
...
// test evaluation of hash index operator expressions
